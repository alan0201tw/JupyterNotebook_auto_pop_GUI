...
class Git_Commit (QWidget):
    def __init__(self , parent = None):
        super(Git_Commit, self).__init__(parent)
        self.create_layout()
        self.create_conect()

    def create_layout(self):
        column1 = QHBoxLayout()
        self.intro = QLabel("Commit to Git?")
        self.commit_message = QLineEdit("Default Commit Message")
        column1.addWidget(self.intro)
        column1.addWidget(self.commit_message)
        ...
        column2 = QHBoxLayout()
        self.info = QPushButton("Git information")
        self.yes = QPushButton("Yes")
        self.no = QPushButton("No")
        column2.addWidget(self.info)
        column2.addWidget(self.yes)
        column2.addWidget(self.no)

        lay = QVBoxLayout()
        lay.addLayout(column1)
        lay.addLayout(column1_1)
        lay.addLayout(column2)

        self.setWindowTitle("Git_Commit")
        self.setLayout(lay)
    ...
    def show_git_info(self):
        repo = Repo('.')
        repo_msg = QMessageBox()
        repo_msg.setWindowTitle("Git info")
        repo_msg.setText(repo.git.status())
        repo_msg.exec_()

    def create_conect(self):
        self.yes.clicked.connect(self.commit)
        self.no.clicked.connect(self.close)
        self.info.clicked.connect(self.show_git_info)
